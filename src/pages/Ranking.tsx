...
import { Trophy, Flame, Medal, Crown, RotateCcw, AlertTriangle, Users } from 'lucide-react';
...
export const Ranking: React.FC = () => {
  ...
  const [abaAtiva, setAbaAtiva] = useState<'geral' | 'grupos'>('geral');
  ...
  return (
    <div className="p-6 pt-12 max-w-2xl mx-auto">
      <header className="mb-6 flex items-center justify-between">
        ...
        <Trophy size={48} className="text-[#FFD700] opacity-10 shrink-0" />
      </header>

      {/* Seletor: Ranking Geral (da academia) x Grupos (criados pelos alunos) */}
      <div className="flex gap-2 mb-8 p-1 bg-zinc-900/60 border border-zinc-800 rounded-2xl">
        <button
          ...
          className={cn(
            ...
            abaAtiva === 'geral' ? 'bg-[#FFD700] text-black' : 'text-zinc-500'
          )}
        >
          <Trophy size={13} /> Geral
        </button>
        <button
          onClick={() => setAbaAtiva('grupos')}
          className={cn(
            'flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl font-black uppercase text-[10px] tracking-wider transition-colors',
            abaAtiva === 'grupos' ? 'bg-[#FFD700] text-black' : 'text-zinc-500'
          )}
        >
